...
def encrypt_with_aes(input_string, password, salt):
    key = generate_aes_key(password, salt)  # derive key
    f = Fernet(key)  # init fernet
...
salt = b'Tandon' # Remember it should be a byte-object  # byte salt value
password = "mrt440@nyu.edu"  # the nyu email address
input_string = "AlwaysWatching"  # secret plaintext
...
dns_records = {
    ...
    'safebank.com.': {dns.rdatatype.A: '192.168.1.102'},  # record entry 1
    'google.com.': {dns.rdatatype.A: '192.168.1.103'},  # record entry 2
    'legitsite.com.': {dns.rdatatype.A: '192.168.1.104'},  # record entry 3
    'yahoo.com.': {dns.rdatatype.A: '192.168.1.105'},  # record entry 4

    'nyu.edu.': {  # nyu domain record entry 5
...
def run_dns_server():
    ...
    server_socket.bind(('127.0.0.1', 53))  # bind the local host DNS
    ...
    while True:
        try:
            ...
            request = dns.message.from_wire(data)  # parse the DNS request
            # Create a response message using the `dns.message.make_response` method
            response = dns.message.make_response(request)  

            # Get the question from the request
            question = request.question[0]  # first question
            ...
            server_socket.sendto(response.to_wire(), addr)  # send DNS response
